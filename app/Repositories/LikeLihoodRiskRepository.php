...
/**
 * Class LikeLihoodRiskRepository.
 */
class LikeLihoodRiskRepository extends RiskRepository
{
   /**
    * Get risk by id
    */
   ...
   function getRiskCount($option = []) {
      $query = DB::table("rms_add_risk_treatment_likelihood AS RAT")
      ->join('rms_risk_register AS RR', 'RAT.Risk_ID', '=', 'RR.Risk_ID')
      ->select([
         DB::raw('COUNT(RAT.Risk_ID) AS count'),
         'LikelihoodStatus AS status'
      ])
      ->groupBy('LikelihoodStatus');
    ...
    */
    function getRiskTreatmentByCategories($option = []) {
      ...
      ->select([
         DB::raw('COUNT(RAT.Risk_ID) AS count'),
         'LikelihoodAddCtrlCategory AS category'
      ])
      ->groupBy('LikelihoodAddCtrlCategory');
   ...
   function getRiskTreatmentDetails($option = []) {
      ...
      ->select(
         ...
         'LikelihoodAddCtrlCategory AS category',
         'LikelihoodAddCtrlDescription AS description',
         'LikelihoodPIC AS pic',
         DB::raw("DATE_FORMAT(LikelihoodDueDate, '%d/%m/%Y') AS dueDate"),
         DB::raw("FORMAT(LikelihoodCost, 2) AS cost"),
         'LikelihoodStatus AS status',
         'LikelihoodRemarks AS remarks'
      )
      ->join('rms_risk_register as RR', 'RAT.Risk_ID', '=', 'RR.Risk_ID')
      ->where('LikelihoodAddCtrlCategory', '<>', 'Accept')
